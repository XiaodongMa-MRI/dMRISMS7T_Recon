...
% GCC based compressions
    ...
    % xiaodong change for the situation of Nx_nav ~= Nx; changed back
...
%     if Nx ~= size(mtx,1)
%         Nfactor = size(mtx,3)/Nx;
%         Ncenter_data = floor(Nx/2)+1;
%         Ncenter_mtx = floor(size(mtx,3)/2)+1;
%         for n=1:Nx
%             tmpc = reshape(squeeze(im(n,:,:,:)),[Ny*Nz,Nc]);
% %             n_mtx = round( (n-Ncenter_data)*Nfactor+Ncenter_mtx );
%             n_mtx = round( (n-1)*Nfactor+1 );
%             res(n,:,:,:) = permute(reshape(tmpc*mtx(:,:,n_mtx),[Ny,Nz,ncc]),[4,1,2,3]);
%         end
%     else
%         for n=1:Nx
%             tmpc = reshape(squeeze(im(n,:,:,:)),[Ny*Nz,Nc]);
